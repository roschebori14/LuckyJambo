-- Lucky Jambo - Dots and Boxes
--
-- Second game built on the boardgame.io rules engine (see
-- lib/games/dots-and-boxes/game.ts + engine.ts), following the exact
-- same pattern as Four in a Row (050_four_in_a_row.sql): the move
-- itself is validated and computed in TypeScript, this RPC is the
-- trusted persistence + settlement layer (re-checks turn order and
-- optimistic concurrency, then persists the already-validated result
-- and settles/refunds).
--
-- Seating convention (matches every other 2-seat game): creator is
-- always 'R' and moves first; joiner is seated 'Y'.
--
-- Grid: 4x4 boxes (5x5 dots) - 20 horizontal lines + 20 vertical lines,
-- 16 boxes total. See lib/games/dots-and-boxes/game.ts's header comment
-- for the exact line/box indexing scheme.
...
-- ---------------------------------------------------------------
-- Atomic, authoritative persistence for dots-and-boxes moves.
-- DotsAndBoxesGame's drawLine (boardgame.io rules, run headlessly in
-- app/api/dots-and-boxes/move/route.ts) decides legality/outcome; this
-- RPC re-checks turn order and that the board hasn't changed since the
-- client last read it (p_expected_h_lines/p_expected_v_lines), then
-- persists the validated result and settles (decisive win) or refunds
-- (a tied box count is a real, reachable draw) if the game ended.
-- ---------------------------------------------------------------
create or replace function public.apply_dots_and_boxes_move_result(
  p_match_id uuid,
  p_expected_h_lines jsonb,
  p_expected_v_lines jsonb,
  p_new_h_lines jsonb,
  p_new_v_lines jsonb,
  p_new_box_owners jsonb,
  p_new_scores jsonb,
  p_new_turn text,
  p_winner text,
  p_is_draw boolean,
...
begin
  ...
  if v_state->'h_lines' != p_expected_h_lines or v_state->'v_lines' != p_expected_v_lines then
  ...
  v_new_state := jsonb_build_object(
    'h_lines', p_new_h_lines,
    'v_lines', p_new_v_lines,
    'box_owners', p_new_box_owners,
    'scores', p_new_scores,
...
revoke execute on function public.apply_dots_and_boxes_move_result(uuid, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, text, text, boolean, boolean) from public;
revoke execute on function public.apply_dots_and_boxes_move_result(uuid, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, text, text, boolean, boolean) from anon;
grant execute on function public.apply_dots_and_boxes_move_result(uuid, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, text, text, boolean, boolean) to authenticated;

insert into games (name, slug, min_stake, max_stake, is_active)
values ('Dots and Boxes', 'dots-and-boxes', 50, 100000, true)
