...
import { LayoutDashboard, Gamepad2, Swords, Wallet, Users, User, Menu, X, Shield, Trophy, Bell } from "lucide-react";
    ...
    <>
      ...
      {open && (
        <div className="border-b shadow-2xl md:hidden" style={{ background: "var(--lj-navy-2)", borderColor: "var(--lj-border)" }}>
          <nav className="flex flex-col gap-1 p-3">
            {[...NAV, { href: "/profile", icon: User, label: "Profile" }, { href: "/notifications", icon: Bell, label: "Notifications" }].map(({ href, icon: Icon, label }) => (
              <Link key={href} href={href} onClick={() => setOpen(false)}
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${active(href) ? "lj-nav-active text-white" : "text-[var(--lj-muted)] hover:bg-white/5 hover:text-white"}`}>
                <Icon size={18}/> {label}
              </Link>
            ))}
            {isAdmin && (
              <Link href="/admin" onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-yellow-400 hover:bg-yellow-400/10">
      ...
      <aside className="hidden min-h-screen w-60 flex-shrink-0 flex-col md:flex"
        ...
        <nav className="flex flex-col gap-1 px-3 pb-4">
          ...
          <p className="mb-1 mt-4 px-3 text-[10px] font-semibold uppercase tracking-widest text-[var(--lj-muted)]">Account</p>
          {[{ href: "/notifications", icon: Bell, label: "Notifications" }, { href: "/profile", icon: User, label: "Profile" }].map(({ href, icon: Icon, label }) => (
            <Link key={href} href={href}
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${active(href) ? "lj-nav-active text-white" : "text-[var(--lj-muted)] hover:bg-white/5 hover:text-white"}`}>
              <Icon size={18}/> {label}
            </Link>
          ...
          {isAdmin && (
            <>
              ...
              <Link href="/admin" className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${active("/admin") ? "lj-nav-active text-yellow-300" : "text-yellow-400/70 hover:bg-yellow-400/10 hover:text-yellow-300"}`}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden"
        ...
        <div className="grid grid-cols-6">
          {BOTTOM_NAV.map(({ href, icon: Icon, label }) => (
            <Link key={href} href={href}
              className={`flex flex-col items-center gap-0.5 py-2.5 text-[10px] font-medium transition-colors ${active(href) ? "text-[var(--lj-cyan)]" : "text-[var(--lj-muted)]"}`}>
