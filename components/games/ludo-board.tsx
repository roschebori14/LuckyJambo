...
const COLOR_LETTER: Record<LudoColor, string> = {
  red: "R",
  green: "G",
  yellow: "Y",
  blue: "B",
...
const ENTRY_OFFSET: Record<LudoColor, number> = {
  red: 1,
  green: 14,
  yellow: 27,
  blue: 40,
...
const STAR_ABS = new Set<number>(
  (Object.keys(ENTRY_OFFSET) as LudoColor[]).flatMap((c) => [
    ENTRY_OFFSET[c],
    (ENTRY_OFFSET[c] + 8) % 52,
  ]),
);
...
export default function LudoBoard({ matchId, userId }: LudoBoardProps) {
  ...
  useEffect(() => {
    ...
    try {
      (
        sceneRef.current as unknown as {
          renderTokens: (s: LudoState, m: number[], seat: number) => void;
        }
      ).renderTokens(state, state.movable_tokens, mySeat);
    } catch (err) {
      console.error("Ludo renderTokens failed:", err);
    }
    (
      sceneRef.current as unknown as { onTokenTap?: (i: number) => void }
    ).onTokenTap = async (tokenIndex: number) => {
      try {
  ...
  const currentName = currentSeatObj
    ? (usernames.get(currentSeatObj.user_id) ?? "Player")
    : "…";
  ...
  async function forfeit() {
    if (
      !window.confirm(
        "Are you sure? You'll forfeit your stake and leave the match.",
      )
    )
      return;
    setForfeiting(true);
    ...
    <div className="flex flex-col items-center gap-3">
      ...
      <div className="flex w-full items-center justify-between px-1 text-sm">
        ...
        {mySeatObj && (
          <span className="text-xs font-semibold text-[var(--lj-muted)]">
            You're{" "}
            <span
              style={{ color: `#${COLOR_HEX[mySeatObj.color].toString(16)}` }}
            >
              {mySeatObj.color}
            </span>
          </span>
      ...
      <div className="flex items-center gap-3">
        {(rollDisplay != null || state.dice_value != null) && (
          <span
            className={`flex h-10 w-10 items-center justify-center rounded-lg bg-white text-lg font-black text-black ${rolling ? "animate-bounce" : ""}`}
          >
