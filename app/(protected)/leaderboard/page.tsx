...
export default async function LeaderboardPage() {
  ...
  const Table = ({ rows, valueKey, valueSuffix }: { rows: (WinRow | EarnerRow)[]; valueKey: "wins" | "earned"; valueSuffix: string }) => (
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div>
          ...
          {wins.length > 0 ? <Table rows={wins} valueKey="wins" valueSuffix="W" /> : <div className="lj-card p-8 text-center text-sm text-[var(--lj-muted)]">No completed matches yet</div>}
        </div>
        <div>
          ...
          {earners.length > 0 ? <Table rows={earners} valueKey="earned" valueSuffix="XAF" /> : <div className="lj-card p-8 text-center text-sm text-[var(--lj-muted)]">No earnings yet</div>}
        </div>
      </div>
    </div>
  );
